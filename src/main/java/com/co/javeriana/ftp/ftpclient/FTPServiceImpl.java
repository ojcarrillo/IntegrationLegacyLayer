...
import com.co.javeriana.ftp.exceptions.ErrorMessage;
...
public class FTPServiceImpl implements FTPService {
     ...
     */
    @Override
    public void connectToFTP(String host, Integer port, String user, String pass) throws FTPErrors {
    ...
    public void downloadFileFromFTP(String ftpRelativePath, String copytoPath) throws FTPErrors {
        ...
        try {
            fos = new FileOutputStream(new File(copytoPath));
    ...
    @Override
    public String getFtpWorkingDir() throws FTPErrors {
        if (this.ftpconnection.isConnected()) {
            try {
                return this.ftpconnection.printWorkingDirectory();
            } catch (IOException f) {
               throw new FTPErrors( new ErrorMessage(-9, "Ha ocurrido un error al realizar la desconexión del servidor FTP"));
            }
        }
        return null;
    }
    ...
    @Override
    public FTPFile[] getFilesFromFTPDir(String remoteDir) throws FTPErrors {
        if (this.ftpconnection.isConnected()) {
            try {
                return this.ftpconnection.listFiles(remoteDir);
            } catch (IOException f) {
               throw new FTPErrors( new ErrorMessage(-10, "Ha ocurrido un error al realizar la desconexión del servidor FTP"));
            }
        }
        return null;
    }
    ...
    @Override
    public void deleteFileFromFTPDir(String fileName) throws FTPErrors {
        if (this.ftpconnection.isConnected()) {
        	try {
                this.ftpconnection.deleteFile(fileName);
            } catch (IOException f) {
               throw new FTPErrors( new ErrorMessage(-11, "Ha ocurrido un error al realizar la desconexión del servidor FTP"));
            }
        }
    }
    
    @Override
    public void downloadAndDeleteFilesFromFTPDir(String remoteDirPath, String localDirPath) throws FTPErrors {
    	try {
			FTPFile[] listFiles = getFilesFromFTPDir(remoteDirPath);
			if (listFiles == null || listFiles.length == 0) {
		        return;
		    }
			for (FTPFile fileSel : listFiles) {
				if (fileSel.getName().equals(".") || fileSel.getName().equals("..")) {
					continue;
				}
				downloadFileFromFTP(remoteDirPath + "/" + fileSel.getName(), localDirPath + fileSel.getName());
				deleteFileFromFTPDir(remoteDirPath + "/" + fileSel.getName());
			}
    	} catch (FTPErrors e) {
    		throw new FTPErrors( new ErrorMessage(-12, "Ha ocurrido un error al realizar la desconexión del servidor FTP"));
		}
    }
}
